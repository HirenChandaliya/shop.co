...
import 'package:http/http.dart' as http;
import 'package:myecommerce/modal/productModel.dart'; // Tamari model file nu path
...
class ApiService {
  ...
  // 2. Specific Category ni Products Lavo
  // Platzi ma category ID thi male che, pan string thi filter karva mate
  // apde badhi products lavi ne filter kari shakiye (simple way)
  static Future<List<Product>> getProductsByCategory(String categoryName) async {
    try {
      // Pela badhi products lavo
      final allProducts = await getAllProducts();

      // Pachhi dart ma filter karo (Kem ke API ma category ID joie)
...
// lib/services/api_service.dart
  ...
  // 5. Login User
  // Platzi ma 'username' nai pan 'email' vapray che
  static Future<String> login(String email, String password) async {
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) { // Platzi returns 201 created
        final data = json.decode(response.body);
        return data['access_token']; // Platzi returns 'access_token'
      } else {
...
// ==========================================
// NOTE: Platzi API pase 'Cart' mate koi database endpoint nathi.
// Cart tamare app ma 'Provider' ke 'Local Variable' thi manage karvu padse.
// ==========================================

/* static Future<Map<String, dynamic>> addToCart(...) async {
     // A functionality Platzi API ma Server side nathi.
     // Tamare local list ma add karvu padse.
  }
  */
}